...
def parse_request_payload(d: dict) -> dict:
    return {
        ...
        "hosei1": parse_float(d.get("hosei1", 0)), # ★引継ぎ: 補正値1
        ...
        "hosei2": parse_float(d.get("hosei2", 0))  # ★引継ぎ: 補正値2
    }
...
def build_average_temperature(lat: float, lon: float, fiscal_year: int, n_years: int = 3) -> pd.DataFrame:
    ...
    if not dates: return pd.DataFrame(columns=["month_day", "tave_avg"])
...
def build_this_year_dataframe(lat: float, lon: float, fiscal_year: int, today: date, df_avg: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.merge(df_avail.drop(columns=["date"]), on="date_ts", how="left")
    df["month_day"] = df["date_ts"].dt.strftime("%m-%d")
    
    if not df_avg.empty: df = df.merge(df_avg[["month_day", "tave_avg"]], on="month_day", how="left")
    else: df["tave_avg"] = 10.0

    mask_normal = df["tag"] == "normal"
    
    df["tave_this"] = df["tave_real"]
    df.loc[mask_normal, "tave_this"] = np.nan
    df["tave_this"] = df["tave_this"].astype(float).fillna(df["tave_avg"]).round(1)

    df["prcp_this"] = df["prcp_real"]
    df.loc[mask_normal, "prcp_this"] = 0.0
    df["prcp_this"] = df["prcp_this"].astype(float).fillna(0.0).round(1)

    # ★引継ぎ: 最高・最低気温は実測値のみを使用（平年値で埋めない）
    df["tmax_this"] = df["tmax_real"].round(1)
    df["tmin_this"] = df["tmin_real"].round(1)

    df.drop(columns=["month_day", "date_ts", "tave_avg", "tave_real", "tmax_real", "tmin_real", "prcp_real"], inplace=True)
    return df

def load_daylength_table(csv_path: Path = DL_CSV_PATH) -> pd.DataFrame:
    if not csv_path.exists(): raise FileNotFoundError(f"Daylength CSV not found")
...
def build_accumulation_dataframe(df_src: pd.DataFrame, start_date: date, end_date: date, method: int, t_base: float, t_ceiling, target_gdd: float, df_dl_master: pd.DataFrame, hosei: float):
    if not start_date or not end_date: return pd.DataFrame(), {}, {}
    validate_method_and_thresholds(method, t_base, t_ceiling)
    if df_src.empty: return df_src.copy(), {}, {}
    ...
    # 補正計算
    try:
        df["corr_cum_ct"] = df["cum_ct"] + hosei
        df["abs_diff_corr"] = (df["corr_cum_ct"] - target_gdd).abs().round(1)
        row_corr = df.loc[df["abs_diff_corr"].idxmin()]
        corrected = {
            "date": row_corr["date"].isoformat(), "cum_ct": round_or_none(row_corr["corr_cum_ct"], 1)
        }
    except Exception:
        corrected = {}
        
    return df, closest, corrected
...
def get_climate_data():
    try:
        ...
        df_ct1, closest1, corr1 = build_accumulation_dataframe(df_this, params["ct1_start"], params["ct1_end"], params["method1"], params["base_threshold1"], params["ceiling_threshold1"], params["gdd1_target"], df_dl_master, params["hosei1"])
        ...
        df_ct2, closest2, corr2 = build_accumulation_dataframe(df_this, ct2_start_calc, ct2_end_calc, params["method2"], params["base_threshold2"], params["ceiling_threshold2"], params["gdd2_target"], df_dl_master, params["hosei2"])
        ...
        return jsonify(replace_nan_with_none({
            ...
            "ct1": dataframe_to_records_with_iso_date(df_ct1), "gdd1_target": closest1, "gdd1_target_corr": corr1,
            "ct2": dataframe_to_records_with_iso_date(df_ct2), "gdd2_target": closest2, "gdd2_target_corr": corr2,

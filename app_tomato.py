...
def get_cached_avg_data(year, lat, lon):
    start_str, end_str = f"{year}-04-01", f"{year+1}-03-31"
...
def get_climate_data():
    try:
        ...
        if not df_cy_t.empty or not df_cy_p.empty:
            ...
            for c, r in [("TMP_mea", "tave_real"), ("APCPRA", "prcp_real")]:
                if c not in df_cy.columns: df_cy[c] = np.nan
                df_cy.rename(columns={c: r}, inplace=True)
        ...
        mask_f = (df_available["date"] >= (today - pd.Timedelta(days=1))) & (df_available["date"] <= (today + pd.Timedelta(days=7))) if not df_available.empty else pd.Series(False, index=df_this.index)
        df_forecast = df_available.loc[mask_f].copy().rename(columns={"tave_real": "tave_this", "prcp_real": "prcp_this"}) if mask_f.any() else pd.DataFrame(columns=["date", "tave_this", "prcp_this"])
        ...
        def clean(obj):
            ...
            return "" if (isinstance(obj, float) and np.isnan(obj)) or obj is None else (obj.strftime("%Y-%m-%d") if isinstance(obj, pd.Timestamp) else obj)
